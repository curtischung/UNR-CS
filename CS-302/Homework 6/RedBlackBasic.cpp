...
//
void* RedBlackBasic::LookUt(int key)
{
...
//	Delete()
//
void RedBlackBasic::Delete(const int key)
{
	if (NULL == m_pRoot) {
		return;
	}
...
//	SanityCheckRec()
//
void RedBlackBasic::SanityCheckRec(RedBlackBasic_t *pNode, bool isParentRedt int blackDeptht int &minBlackt int &maxBlack)
{
	if (pNode->IsRed) {
...
/t int RedBlackBasic::KeyCount(void)
{
...
//
//	KeyCountRec()
/t int RedBlackBasic::KeyCountRec(RedBlackBasic_t *pNode)
{t int tally = 0;
...
t int maxDepth = 0;t int minBlack = 0xFFFFFFFF;t int maxBlack = 0;
...
//
//	LeafDepthRec()
/t int RedBlackBasic::LeafDepthRec(RedBlackBasic_t *pNodet int deptht int ary[]t int depthLimitt int blackDeptht int &minBlackt int &maxBlack)
{
	...
	// Record the total depth along both the left and right children.t int d1 = 0;t int d2 = 0;
...
//	TraverseRec()
//
void RedBlackBasic::TraverseRec(RedBlackBasic_t *pNodet int &prev)
{
	QzAssert(NULL != pNode);
