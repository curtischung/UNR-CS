...
void LeftLeaningRedBlack::LeafDepth(void)
{
	...
	for (int i = 0; i < maxDepth; ++i) {
		...
		std::cout << i << " " << ary[i] << " " << tally;
	}

	std::cout << "black depth: " << minBlack << " to " << maxBlack;
...
bool LeftLeaningRedBlack::printParentRec(LLTB_t *pNode, int key) {
    ...
    if (printParentRec(pNode->pLeft, key) || printParentRec(pNode->pRight, key)){
        std::cout << "P: " << pNode->Ref.Key << " ";
    }
