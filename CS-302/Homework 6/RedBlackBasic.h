...
public:
	...
	void SanityCheckRec(RedBlackBasic_t *pNode, bool isParentRed, int blackDepth, int &minBlack, int &maxBlack);

	U32  KeyCount(void);
	U32  KeyCountRec(RedBlackBasic_t *pNode);

	void LeafDepth(void);
	U32  LeafDepthRec(RedBlackBasic_t *pNode, int depth, int ary[], int depthLimit, int blackDepth, int &minBlack, int &maxBlack);

	void Traverse(void);
	void TraverseRec(RedBlackBasic_t *pNode, int &prev);
};
